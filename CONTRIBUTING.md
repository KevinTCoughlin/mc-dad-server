...
Click **Code → Codespaces → New codespace** on the repo page. The dev container comes pre-configured with Go 1.26, Java 21, Kotlin, and all required tooling — zero local setup. The repo prefers `go1.26.4` and remains compatible with `go1.25.11`.
...
|------|---------|---------|
| [Go](https://go.dev/dl/) | 1.25.11+ (`go1.26.4` recommended) | Build the CLI binary |
...
Containerfile          Multi-stage container build (Eclipse Temurin 25 JRE on Alpine Linux)
