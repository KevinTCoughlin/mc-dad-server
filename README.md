...
Run the server in Docker or Podman instead of a bare-metal screen session. The Containerfile uses Eclipse Temurin 25 JRE on Alpine Linux and includes Geyser, Floodgate, Parkour, Multiverse, and WorldEdit.
...
**Quickest path:** Open in [GitHub Codespaces](https://github.com/codespaces) — the dev container includes Go 1.26, Java 21, Kotlin, and all dev tools pre-configured. The repo prefers `go1.26.4` and remains compatible with `go1.25.11`.

**Local prerequisites:** Go `1.25.11+` with `go1.26.4` recommended, [just](https://github.com/casey/just#installation), and optionally Java 21+ and Kotlin for plugin work. Run `just tools` to install the Go dev tools (golangci-lint, gofumpt, goreleaser).
